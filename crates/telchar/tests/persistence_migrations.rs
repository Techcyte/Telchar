...
fn latest_migration_version_matches_resulting_schema() {
    ...
    assert_eq!(outcome.resulting_version, 18);
}
...
fn empty_database_migrates_to_minimum_lifecycle_schema() {
    ...
    assert_eq!(ledger[16].get::<_, i64>(0), 17);
    assert_eq!(ledger[16].get::<_, String>(1), "reconciled_store_leases");
    assert_eq!(ledger[16].get::<_, Vec<u8>>(2).len(), 32);
    assert_eq!(ledger[17].get::<_, i64>(0), 18);
    assert_eq!(
        ledger[17].get::<_, String>(1),
        "shared_build_attempt_trace_context"
    );
    assert_eq!(ledger[17].get::<_, Vec<u8>>(2).len(), 32);
...
fn output_retention_migration_backfills_version_one_rows() {
    ...
    assert_eq!(outcome.applied_this_run, 17);
    let mut client = fixture.connect();
...
fn rerunning_an_exact_prefix_is_idempotent() {
    ...
    assert_eq!(first.applied_this_run, 18);
    assert_eq!(second.previously_applied, 18);
...
fn future_schema_version_is_rejected() {
    ...
    fixture
        .connect()
        .execute(
            "INSERT INTO telchar_schema_migrations (version, name, checksum) VALUES (19, 'future', decode(repeat('00', 32), 'hex'))",
            &[],
        )
