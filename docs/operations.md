...
The public NixOS module is `nixosModules.default` (also exported as `nixosModules.telchar`). It manages the daemon and its application configuration. PostgreSQL, gateway-store access, OpenSSH, and credential acquisition are operator-owned infrastructure.
...
Provision infrastructure explicitly with ordinary NixOS options: `services.postgresql`, `services.openssh`, `nix.settings.trusted-users`, `systemd.tmpfiles.rules`, and service dependency ordering as appropriate. The module neither creates a database nor assumes a local connection URL. Supply `database.urlFile`, `database.url`, or application database settings.

See [NixOS examples](../examples/nixos/README.md) for complete compositions, including regular OpenSSH and external-database deployment. No standalone-host profile is exported.
...
Operator-owned SSH ingress must enable OpenSSH authentication metadata (`ExposeAuthInfo yes`) and invoke the command generated by `lib.sshForcedCommand`. Public keys produce fingerprint-based credential IDs. Certificates produce `ssh-cert:<CA-byte-length>:<CA-fingerprint>:<key-ID-byte-length>:<key-ID>` credential IDs, with certificate principals retained as metadata. NixOS and OCI ingress use the same extraction and normalization. Explicit identity mappings override audit and quota subjects; otherwise the first certificate principal supplies the audit subject and the credential ID supplies the quota subject. Keep authentication files operator-owned and restricted to the Telchar account.

Server identity and client authentication are independent. OpenSSH `HostCertificate` supplies server identity; `TrustedUserCAKeys` and `AuthorizedPrincipalsFile` configure client certificate authentication. Authorized keys may coexist with a client CA. Configure these through operator-owned OpenSSH settings.
...
Vault/AWS authentication, SSH certificate issuance, and atomic credential installation belong to the deployment repository. The [advanced NixOS example](../examples/nixos/README.md#aws-vault-external-postgresql-and-nomad-workers) illustrates this composition. Telchar consumes protected credential files and supports contents-only Nomad token reload; it does not acquire infrastructure credentials.
...
Ingress shutdown and restart behavior follows the operator-owned OpenSSH/systemd configuration. Stop or disable that ingress explicitly when maintenance requires admission to remain closed.
