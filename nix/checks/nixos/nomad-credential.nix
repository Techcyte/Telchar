# Verifies Telchar consumes a protected Nomad token through an external backend request.
{
  pkgs,
  system,
  telchar,
...
let
  ...
  '';
  buildDerivation = pkgs.writeText "nomad-credential-build.nix" ''
    derivation {
      name = "nomad-credential-build";
...
pkgs.testers.nixosTest {
  name = "telchar-nixos-nomad-credential";
  ...
  testScript = ''
    ...
    client.succeed("cp ${buildDerivation} /tmp/nomad-credential-build.nix")
    derivation_path = client.succeed("nix-instantiate /tmp/nomad-credential-build.nix").strip()
    ...
    nomad_api.wait_until_succeeds("test $(cat /tmp/observed-token) = protected-nomad-token", timeout=60)
    client.succeed("kill $(cat /tmp/build.pid) || true")
    gateway.fail("nix-store -qR /run/current-system | xargs grep -I -l -m1 protected-nomad-token 2>/dev/null")
  '';
}
