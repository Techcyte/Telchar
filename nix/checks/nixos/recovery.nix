  ...
  nixos-remote-build-contract =
    ...
    harness.mkRestrictedIngressTest {
      ...
      testScript = ''
        ...
        gateway.wait_until_succeeds("journalctl -u telchar-daemon.service --no-pager | grep -q 'worker.build_derivation.cached'")
        gateway.succeed("sudo -u postgres psql -d telchar-ingress -v ON_ERROR_STOP=1 -Atc \"SELECT count(*) FROM store_leases WHERE owner_kind = 'request' AND purpose = 'input' AND state = 'released' AND released_at IS NOT NULL\" | grep -qx 6")
        gateway.succeed("sudo -u postgres psql -d telchar-ingress -v ON_ERROR_STOP=1 -Atc \"SELECT count(*) FROM store_leases WHERE owner_kind = 'request' AND purpose = 'derivation' AND state = 'released' AND released_at IS NOT NULL\" | grep -qx 1")
        ...
        gateway.succeed("test \"$(wc -l < /tmp/telchar-input-leases)\" -eq 6")
