...
let
  ...
  telcharImageReference = "${telcharImage.imageName}:${telcharImage.imageTag}";
  nomadWorkerImageReference = "${nomadWorkerImage.imageName}:${nomadWorkerImage.imageTag}";
in
{
  nixos-oci-gateway =
    let
    ...
    pkgs.testers.nixosTest {
      ...
      testScript = ''
        ...
        gateway.wait_until_succeeds("test $(sudo -u postgres psql -d telchar -Atc \"select count(*) from singleton_ownership where owner_kind = 'daemon' and lease_expires_at > clock_timestamp()\") -eq 0", timeout=30)
        gateway.succeed("docker run -d --name telchar-daemon --network host --user 995:995 -e HOME=/var/lib/telchar -e TELCHAR_CONFIG=/etc/telchar/telchar.toml -e TELCHAR_DATABASE_URL=postgresql://telchar@localhost/telchar -e TELCHAR_GATEWAY_DISK_RESERVE_BYTES=1048576 -e TELCHAR_GATEWAY_STORE_URI=unix:///nix/var/nix/daemon-socket/socket -e TELCHAR_GATEWAY_GC_ROOT_DIRECTORY=/var/lib/telchar/gc-roots -e TELCHAR_SINGLETON_CHECK_INTERVAL_MS=100 -e TMPDIR=/var/lib/telchar/import -v /etc/telchar-oci:/etc/telchar:ro -v /run/telchar-oci:/run/telchar -v /var/lib/telchar/import:/var/lib/telchar/import -v /var/lib/telchar/gc-roots:/var/lib/telchar/gc-roots -v /nix/var/nix/daemon-socket/socket:/nix/var/nix/daemon-socket/socket ${telcharImageReference} daemon --socket /run/telchar/daemon.sock --frontend-uid 995")
        gateway.wait_until_succeeds("test -S /run/telchar-oci/daemon.sock && docker inspect -f '{{.State.Running}}' telchar-daemon | grep -qx true", timeout=30)
        ...
        gateway.succeed("test \"$(docker inspect -f '{{.State.ExitCode}}' telchar-daemon)\" -eq 1")
        gateway.succeed("docker logs telchar-daemon > /tmp/telchar-daemon.log 2>&1 && grep -Eq 'database.singleton_ownership.lost|gateway.output_retention.reconciled.*result=.?failed|gateway store retention failed' /tmp/telchar-daemon.log")
        gateway.succeed("docker rm telchar-daemon >/dev/null")
        ...
        output_path = client.succeed(command).strip()
        gateway.succeed("test $(sudo -u postgres psql -d telchar -Atc 'select count(*) from shared_build_attempts') -eq 9")
        image_id = gateway.succeed("docker image inspect ${telcharImageReference} --format '{{.Id}}'").strip()
        gateway.succeed("sudo -u postgres pg_dump -Fc -f /tmp/telchar-before-redeployment.dump telchar")
        gateway.succeed("docker stop --time 10 telchar-daemon >/dev/null && docker rm telchar-daemon >/dev/null && docker image rm ${telcharImageReference} >/dev/null && docker load < ${telcharImage} >/dev/null")
        gateway.wait_until_succeeds("test $(sudo -u postgres psql -d telchar -Atc \"select count(*) from singleton_ownership where owner_kind = 'daemon' and lease_expires_at > clock_timestamp()\") -eq 0", timeout=30)
        assert gateway.succeed("docker image inspect ${telcharImageReference} --format '{{.Id}}'").strip() == image_id
        gateway.succeed("docker run -d --name telchar-daemon --network host --user 995:995 -e HOME=/var/lib/telchar -e TELCHAR_CONFIG=/etc/telchar/telchar.toml -e TELCHAR_DATABASE_URL=postgresql://telchar@localhost/telchar -e TELCHAR_GATEWAY_DISK_RESERVE_BYTES=1048576 -e TELCHAR_GATEWAY_STORE_URI=unix:///nix/var/nix/daemon-socket/socket -e TELCHAR_GATEWAY_GC_ROOT_DIRECTORY=/var/lib/telchar/gc-roots -e TMPDIR=/var/lib/telchar/import -v /etc/telchar-oci:/etc/telchar:ro -v /run/telchar-oci:/run/telchar -v /var/lib/telchar/import:/var/lib/telchar/import -v /var/lib/telchar/gc-roots:/var/lib/telchar/gc-roots -v /nix/var/nix/daemon-socket/socket:/nix/var/nix/daemon-socket/socket ${telcharImageReference} daemon --socket /run/telchar/daemon.sock --frontend-uid 995")
        gateway.wait_until_succeeds("test -S /run/telchar-oci/daemon.sock && docker inspect -f '{{.State.Running}}' telchar-daemon | grep -qx true", timeout=30)
  ...
  nixos-oci-runtime = pkgs.testers.nixosTest {
    ...
    testScript = ''
      ...
      runtime.succeed("docker image inspect ${telcharImageReference} --format '{{json .Config.Entrypoint}} {{json .Config.Cmd}}' | grep -Fx '[\"/bin/telchar\"] [\"daemon\",\"--socket\",\"/run/telchar/daemon.sock\",\"--frontend-uid\",\"995\"]'")
      runtime.succeed("docker image inspect ${nomadWorkerImageReference} --format '{{json .Config.Entrypoint}} {{json .Config.Cmd}}' | grep -Fx '[\"/bin/telchar-nomad-worker\"] null'")
